...
public class VideoGenTraceExplorer implements ITraceExplorer {
	...
	private List<List<? extends videoGenTrace.States.Value>> getAllValueTraces() {
		...
		for (videoGenTrace.States.videoGen.TracedVideoGen tracedObject : traceRoot.getVideoGen_tracedVideoGens()) {
			result.add(tracedObject.getInitializedSequence());
		}
		return result;
	}
	...
	private List<videoGenTrace.States.Value> getAllStateValues(videoGenTrace.States.State state) {
		...
		for (videoGenTrace.States.videoGen.TracedVideoGen tracedObject : ((videoGenTrace.SpecificTrace) state
			...
			traces.add(tracedObject.getInitializedSequence());
		}
	...
	private void goTo(EObject eObject) {
		if (eObject instanceof videoGenTrace.States.State) {
			...
			for (videoGenTrace.States.VideoGen_initialized_Value value : stateToGo.getVideoGen_initialized_Values()) {
				if (value.getParent() instanceof videoGenTrace.States.videoGen.TracedVideoGen) {
					videoGenTrace.States.videoGen.TracedVideoGen parent_cast = (videoGenTrace.States.videoGen.TracedVideoGen) value
							.getParent();
					boolean toset = value.isInitialized();
					boolean current = ((org.irisa.diverse.videogen.videogenl.videoGen.VideoGen) parent_cast
							.getOriginalObject()).isInitialized();
					if (current != toset) {
						((org.irisa.diverse.videogen.videogenl.videoGen.VideoGen) parent_cast.getOriginalObject())
								.setInitialized((boolean) toset);
					}
				}
			}
