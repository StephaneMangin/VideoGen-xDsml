...
//    }
	 ...
	 * 
 	 ...
 	 */ 
    def static addMetadata(Object clazz){
    	val attributes = new HashMap
    	attributes.put("url", false)
    	attributes.put("duration", false)
    	attributes.put("mimetype", false)
    	clazz.class.declaredFields.forEach[
    		attributes.filter[key, value|!value].forEach[key, value|
	    		if (it.name.equals(key)){
	    			attributes.put(key, true)
	    		} 
    		]
    	]
    	if (!attributes.values.contains(false)) {
	    	val video = clazz as Video
	        val url = Paths.get(video.url)
			video.duration = VideosHelper.getDuration(url)
...
//«FOR sequence: videogen.entrySequence»
